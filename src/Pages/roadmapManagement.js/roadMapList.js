...
const RoadMapList = ({addNewCategory,closeModal}) => {
  ...
  useEffect(() => {
    ...
    roadMapList(currPage);
  }, [reloadPage, filterData, page, currPage]);
  const roadMapList = async (currPage) => {
    setshowLoader(true);

    try {
      let { data } = await CALL_API.RoadMap.get({
        ...filterData,
        page: currPage
        // role: "USER"
      });
...
return <Grid container rowGap={2} rowSpacing={2}>

{tableData?.map((item,idex)=>{
  return <Grid item xs={12}>
  <RoadMapCard key={item._id} data={item} pageReloader={pageReloader} />
  </Grid>
})}
</Grid>
}
  ...
  //             <a target="_blank" href={row?.url}>Open</a>

  //             </StyledTableCell>
  //           {row?.amount? <StyledTableCell align="center">
  //            {row?.amount} %
