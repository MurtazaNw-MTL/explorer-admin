...
function Support() {
  ...
  useEffect(() => {
    try {
      (async () => {
        setshowLoader(true);
        const { data } = await CALL_API.STATIC.getSupport({
          _id: "64e5e55342d5df606421b54d"
        });
  ...
  const submit = async () => {
    try {
      ...
      const { data } = await CALL_API.STATIC.supportUpdate(formData);
  ...
  return (
    <PageContainer title="Support" description="Support">
