...
export default function EditUserModal({ show, pageReloader,closeModal,data }) {
...
setformData(data)
  ...
  const handleSubmit=async()=>{
    try {
      ...
      const {data}=await CALL_API.USER.put(formData)
      ...
      >
        <Box sx={style}>
          <Grid container rowGap={2}>
            <Grid item xs={12}>
              <Typography textAlign="center" fontSize={20}> Edit User</Typography>
            </Grid>
            <Grid item xs={12}>
...
export function ConfirmationModal({ show,closeModal,message,action }) {
    ...
    <div>
      ...
      >
        <Box sx={style}>
          <Grid container rowGap={2}>
            <Grid item xs={12}>
              <Typography textAlign="center" fontSize={20}> {message}</Typography>
            </Grid>
          
         <Grid item xs={12}>
          <Button  variant="contained" onClick={action} fullWidth>Submit</Button>
         </Grid>
          </Grid>
        </Box>
      </Modal>
